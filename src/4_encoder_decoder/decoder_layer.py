...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1):
        ...
        self.enc_dec_attn = MultiHeadAttention(d_model=d_model, h=num_heads, dropout=dropout)
    ...
    def forward(self, x, enc_output, tgt_mask=None, memory_mask=None):
        ...
        x = self.norm1(x)
        _x = x
        attn_output, _ = self.enc_dec_attn(x, enc_output, enc_output, mask=memory_mask)
        x = _x + self.dropout2(attn_output)
        x = self.norm2(x)
        _x = x
